...
def test_fluxo_completo_usuario():  # teste completo do fluxo do usuário
    ...
    with sync_playwright() as p:  # abre contexto Playwright
        ...
        # 2️⃣ Gera token
        token_resp = request.post(f"https://demoqa.com/Account/v1/GenerateToken", data=json.dumps(payload), headers={"Content-Type": "application/json"})  # faz POST para gerar token
        ...
        assert delete_resp.status == 204  # verifica se usuário foi deletado com sucesso (204 No Content)
        ...
        assert search_resp.status == 401  # valida que o usuário não existe mais (401 Unauthorized)
